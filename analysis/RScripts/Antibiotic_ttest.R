...
root = dir(root, pattern=paste0("Abx-Pancreatic_Antibiotic_ttest"), full.names=TRUE)
...
for (level in levels) {
  ...
  for (i in startTaxaIndex:ncol(norm.NoSmoke)) {
    ...
    if (sum(norm.NoSmoke[,i]!=0) > nrow(norm.NoSmoke)*0.1) {
      ...
      abx.t <- t.test(bug ~ AntibioticExp)
      p_AntibioticExp[index] <- abx.t$p.value
  ...
  write.table(dFrame, paste0(outputLevel, level, "_", study, "_Antibiotic_ttest.tsv"),sep="\t",quote = FALSE, row.names = FALSE)
...
tsvName <- "Antibiotic_ttest_Summary.tsv"
