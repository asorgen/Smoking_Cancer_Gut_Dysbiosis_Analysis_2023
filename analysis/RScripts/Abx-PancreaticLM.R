...
root = dir(root, pattern=paste0("Abx-PancreaticLM"), full.names=TRUE)
...
for (level in levels) {
  ...
  for (i in startTaxaIndex:ncol(norm.post)) {
    ...
    if (sum(norm.post[,i]!=0) > nrow(norm.post)*0.1) {
      ...
      if(mean(norm.post[norm.post$AntibioticExp=="Cocktail",i])>mean(norm.post[norm.post$AntibioticExp=="None",i])){
        AntibioticExp_direction[index]="Higher with antibiotics"
      }
      else if(mean(norm.post[norm.post$AntibioticExp=="Cocktail",i])<mean(norm.post[norm.post$AntibioticExp=="None",i])){
  ...
  PlotOut <- paste0(outputLevel, level, "_", study, "_", term1, "_Post-tumor_LM_boxplots.pdf")
