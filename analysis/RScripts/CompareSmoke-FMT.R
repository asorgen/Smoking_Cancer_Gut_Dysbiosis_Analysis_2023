...
root = dir(root, pattern=paste0("CompareSmoke-FMT"), full.names=TRUE)
...
for (level in levels) {
  ...
  studies <- c("Pre-tumor_SmokeExp", "Post-tumor_SmokeExp")
  studyNames <- c("Smoke-FMT Pre-tumor: Smoke-exposed Recipient", "Smoke-FMT Post-tumor: Smoke-exposed Recipient")
  ...
  for (s in 1:length(studies)) {
    ...
    if (s!=length(studies)) {
      ...
      for (s1 in otherStudies) {
        ...
        xlab=paste0(studyNames[s]," vs. Smoke-free Recipient")
        ylab=paste0(studyNames[s1]," vs. Smoke-free Recipient")
  ...
  studies <- c("Smoke-exposed_TumorImp", "Smoke-free_TumorImp")
  studyNames <- c("Smoke-exposed FMT recipients: Pre-tumor", "Smoke-free FMT recipients: Pre-tumor")
  ...
  pvalAdj <- p.adjust(pval, method = "BH")
  dFrame<-data.frame(indexNum,studyPairs,pval,r, pvalAdj)
  pairOutFile <- paste0(levelDir, level,"_", study, "_SmokeExposure_by_", study1Term,"_PairwiseComparison.tsv")
  write.table(dFrame, pairOutFile, sep="\t", row.names = FALSE)
  ...
  scatPlotOut <- paste0(levelDir, level,"_", study, "SmokeExposure_by_", study1Term, "_scatterPlotsSegments.pdf")
  pdf(scatPlotOut, width = 5, height = 5)
  theme_set(theme_classic(base_size = 9))
  for (i in plotNum) {
    print(plotList)
  ...
  studies.2 <- c("Pre-tumor__MouseType__Recipient_SmokeExpYes", "Pre-tumor__MouseType__Recipient_SmokeExpNo", "Post-tumor__MouseType__Recipient_SmokeExpYes", "Post-tumor__MouseType__Recipient_SmokeExpNo")
  studyNames.2 <- c("Pre-tumor: Smoke-exposed Recipient", "Pre-tumor: Smoke-free Recipient", "Post-tumor: Smoke-exposed Recipient", "Post-tumor: Smoke-free Recipient")
  ...
  for (s in 1:length(studies.2)) {
    ...
    if (s!=length(studies.2)) {
      ...
      for (s1 in otherStudies) {
        ...
        xlab=paste0(studyNames.2[s]," vs. Smoke-exposed Donor")
        ylab=paste0(studyNames.2[s1]," vs. Smoke-exposed Donor")
