...
for (level in levels) {
  ...
  norm$Treatment <- factor(norm$Treatment, levels = c("Control", "Smoke only", "Neomycin only", "Smoke+Neomycin"))
  ...
  s_SmokeExp <- vector()
  s_AntibioticExp <- vector()
  s_Interaction <- vector()
  ...
  for (i in startTaxaIndex:ncol(norm)) {
    ...
    if (sum(norm[,i]!=0) > nrow(norm)*0.1) {
      ...
      s_AntibioticExp[index] <- myLm$coefficients[3]
      s_Interaction[index] <- myLm$coefficients[4]
      ...
      if(mean(norm[norm$AntibioticExp=="Neomycin",i])>mean(norm[norm$AntibioticExp=="None",i])){
        AntibioticExp_direction[index]="Higher in AntibioticExp"
      }
      else if(mean(norm[norm$AntibioticExp=="Neomycin",i])<mean(norm[norm$AntibioticExp=="None",i])){
