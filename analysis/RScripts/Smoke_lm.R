...
root = dir(root, pattern=paste0("Abx-Pancreatic_Smoke_LM"), full.names=TRUE)
...
for (level in levels) {
  ...
  for (i in startTaxaIndex:ncol(norm.NoAbx)) {
    ...
    if (sum(norm.NoAbx[,i]!=0) > nrow(norm.NoAbx)*0.1) {
      ...
      smoke.lm <- lm(bug ~ SmokeExp)
      sum.lm <- summary(smoke.lm)
      
      p_SmokeExp[index] <- sum.lm$coefficients[2,4]
  ...
  write.table(dFrame, paste0(outputLevel, level, "_", study, "_Smoke_LM.tsv"),sep="\t",quote = FALSE, row.names = FALSE)
