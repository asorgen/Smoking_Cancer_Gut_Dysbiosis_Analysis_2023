...
# BioLockJ v1.4.2: ${scriptDir}/MAIN_14_PCoA_NNK-FMT_DonorRecipient-CasevCtrl.sh

export BLJ=/app/biolockj
export HOSTNAME=c6698da27901

pipeDir="/mnt/efs/pipelines/mouseSmoke_analysis_2024Dec09"
modDir="${pipeDir}/14_PCoA_NNK-FMT_DonorRecipient-CasevCtrl"
scriptDir="${modDir}/script"
tempDir="${modDir}/temp"
logDir="${modDir}/log"
outputDir="${modDir}/output"

touch "${scriptDir}/MAIN_14_PCoA_NNK-FMT_DonorRecipient-CasevCtrl.sh_Started"
...
function spawnDockerContainer() {
    ...
    echo "${SCRIPT_ID}:docker:${containerId}" >> ${scriptDir}/MAIN_14_PCoA_NNK-FMT_DonorRecipient-CasevCtrl.sh_Started
    docker inspect ${containerId}
}

function scriptFailed() {
    echo "Line #${2} failure status code [ ${3} ]:  ${1}" >> "${scriptDir}/MAIN_14_PCoA_NNK-FMT_DonorRecipient-CasevCtrl.sh_Failures"
    exit ${3}
}
...
executeLine "spawnDockerContainer ${scriptDir}/14.0_GenMod.sh" ${LINENO}

touch "${scriptDir}/MAIN_14_PCoA_NNK-FMT_DonorRecipient-CasevCtrl.sh_Success"
