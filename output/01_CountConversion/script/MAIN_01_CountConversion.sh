...
function spawnDockerContainer() {
    ...
    echo "${SCRIPT_ID}:docker:${containerId}" >> ${scriptDir}/MAIN_01_CountConversion.sh_Started
    docker inspect ${containerId}
}

function scriptFailed() {
    echo "Line #${2} failure status code [ ${3} ]:  ${1}" >> "${scriptDir}/MAIN_01_CountConversion.sh_Failures"
    exit ${3}
}
...
executeLine "spawnDockerContainer ${scriptDir}/01.0_GenMod.sh" ${LINENO}

touch "${scriptDir}/MAIN_01_CountConversion.sh_Success"
