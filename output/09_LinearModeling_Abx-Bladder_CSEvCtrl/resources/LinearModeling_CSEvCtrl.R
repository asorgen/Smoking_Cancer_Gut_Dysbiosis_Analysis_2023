...
#Date: 02-07-23
#Description: Perform linear regression modeling on case v control 
...
moduleRoot <- paste0("LinearModeling_CSEvCtrl")
  ...
  for (t in tumorStatus) {
    ...
    for (i in 1:ncol(taxa.df)) {
      ...
      if ( mean(bug > 0, na.rm = TRUE) > 0.1 ) {
        ...
        df <- data.frame( bug, Treatment )
        
        myLm <- anova(lm( bug ~ Treatment, data = df ))
        lmSum <- summary(lm( bug ~ Treatment, data = df ))
        ...
        Ctrl <- averages$mean[which(averages$Treatment == "Control")]
        CSE <- averages$mean[which(averages$Treatment == "Smoke only")]
        
        direction[index] <- ifelse(Ctrl > CSE, "Control",
                                    ifelse(Ctrl < CSE, "CSE",
                                           "same"))
    ...
    file.path <- paste0(outputLevel, level, "_by_Treatment_", study, "_", t, "-tumor_LinearModelResults.tsv")
    write.table(dFrame, file=file.path, sep="\t", row.names=FALSE)
    
    file.path <- paste0(outputLevel, level,  "_by_Treatment_", study, "_", t, "-tumor_pValue_Histogram.pdf")
    pdf(file.path)
    
    pValues <- dFrame[,which(colnames(dFrame) == "pValue")]
    n=length(pValues)
    title.lab <- paste0( "Linear Regression ", cancer, " Cancer\n", t, "-tumor Control v. CSE p values (n = ", n, ")")
...
for (level in levels) {
  ...
  for (t in tumorStatus) {
    ...
    lmFileName <- paste0(outputLevel, level, "_by_Treatment_", study, "_", t, "-tumor_LinearModelResults.tsv")
    ...
    plot <- ggplot(dFrame2, aes(x = Taxa, y = logP, fill = direction)) +
      ...
      coord_flip()+
      scale_fill_manual(values = c(Control = "red", CSE = "forestgreen"))+
      theme_minimal() +
    ...
    if (level == "Genus" & study == "Abx-Pancreatic" & t == "Post") {
      file.path <- paste0(outputLevel, "Figure3A_", level,  "_by_Treatment_", study, "_", t, "-tumor_LinearModel_plot.pdf")
    } else if (level == "Genus" & study == "Abx-Pancreatic" & t == "Pre") {
      file.path <- paste0(outputLevel, "FigureS3B_", level,  "_by_Treatment_", study, "_", t, "-tumor_LinearModel_plot.pdf")
    } else if (level == "Genus" & study == "Abx-Colon" & t == "Post") {
      file.path <- paste0(outputLevel, "FigureS3F_", level,  "_by_Treatment_", study, "_", t, "-tumor_LinearModel_plot.pdf")
    } else if (level == "Genus" & study == "Abx-Bladder" & t == "Post") {
      file.path <- paste0(outputLevel, "FigureS3H_", level,  "_by_Treatment_", study, "_", t, "-tumor_LinearModel_plot.pdf")
    } else {
      file.path <- paste0(outputLevel, level,  "_by_Treatment_", study, "_", t, "-tumor_LinearModel_plot.pdf")
    }
