...
# BioLockJ v1.4.2: ${scriptDir}/MAIN_16_LinearModeling_NNK-FMT_CasevCtrl.sh
...
touch "${scriptDir}/MAIN_16_LinearModeling_NNK-FMT_CasevCtrl.sh_Started"
...
function spawnDockerContainer() {
    ...
    echo "${SCRIPT_ID}:docker:${containerId}" >> ${scriptDir}/MAIN_16_LinearModeling_NNK-FMT_CasevCtrl.sh_Started
    docker inspect ${containerId}
}

function scriptFailed() {
    echo "Line #${2} failure status code [ ${3} ]:  ${1}" >> "${scriptDir}/MAIN_16_LinearModeling_NNK-FMT_CasevCtrl.sh_Failures"
    exit ${3}
}
...
executeLine "spawnDockerContainer ${scriptDir}/16.0_GenMod.sh" ${LINENO}

touch "${scriptDir}/MAIN_16_LinearModeling_NNK-FMT_CasevCtrl.sh_Success"
