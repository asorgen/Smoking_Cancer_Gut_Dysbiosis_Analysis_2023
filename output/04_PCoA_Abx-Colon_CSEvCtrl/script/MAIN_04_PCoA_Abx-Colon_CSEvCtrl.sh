...
# BioLockJ v1.4.2: ${scriptDir}/MAIN_04_PCoA_Abx-Colon_CSEvCtrl.sh
...
touch "${scriptDir}/MAIN_04_PCoA_Abx-Colon_CSEvCtrl.sh_Started"
...
function spawnDockerContainer() {
    ...
    echo "${SCRIPT_ID}:docker:${containerId}" >> ${scriptDir}/MAIN_04_PCoA_Abx-Colon_CSEvCtrl.sh_Started
    docker inspect ${containerId}
}

function scriptFailed() {
    echo "Line #${2} failure status code [ ${3} ]:  ${1}" >> "${scriptDir}/MAIN_04_PCoA_Abx-Colon_CSEvCtrl.sh_Failures"
    exit ${3}
}
...
executeLine "spawnDockerContainer ${scriptDir}/04.0_GenMod.sh" ${LINENO}

touch "${scriptDir}/MAIN_04_PCoA_Abx-Colon_CSEvCtrl.sh_Success"
