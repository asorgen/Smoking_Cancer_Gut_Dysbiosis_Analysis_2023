...
# BioLockJ v1.4.2: ${scriptDir}/MAIN_07_LinearModeling_Abx-Pancreatic_CSEvCtrl.sh

export BLJ=/app/biolockj
export HOSTNAME=c6698da27901

pipeDir="/mnt/efs/pipelines/mouseSmoke_analysis_2024Dec09"
modDir="${pipeDir}/07_LinearModeling_Abx-Pancreatic_CSEvCtrl"
scriptDir="${modDir}/script"
tempDir="${modDir}/temp"
logDir="${modDir}/log"
outputDir="${modDir}/output"

touch "${scriptDir}/MAIN_07_LinearModeling_Abx-Pancreatic_CSEvCtrl.sh_Started"
...
function spawnDockerContainer() {
    ...
    echo "${SCRIPT_ID}:docker:${containerId}" >> ${scriptDir}/MAIN_07_LinearModeling_Abx-Pancreatic_CSEvCtrl.sh_Started
    docker inspect ${containerId}
}

function scriptFailed() {
    echo "Line #${2} failure status code [ ${3} ]:  ${1}" >> "${scriptDir}/MAIN_07_LinearModeling_Abx-Pancreatic_CSEvCtrl.sh_Failures"
    exit ${3}
}
...
executeLine "spawnDockerContainer ${scriptDir}/07.0_GenMod.sh" ${LINENO}

touch "${scriptDir}/MAIN_07_LinearModeling_Abx-Pancreatic_CSEvCtrl.sh_Success"
