...
# BioLockJ v1.4.2: ${scriptDir}/MAIN_00_TaxaMetaMerge.sh
...
touch "${scriptDir}/MAIN_00_TaxaMetaMerge.sh_Started"
...
function spawnDockerContainer() {
    ...
    containerId=$(docker run --detach --rm \
     ...
     -e BLJ=/app/biolockj  \
     -e HOSTNAME=4d28528f15d3  \
     asorgen/gg-tidyr:v5 \
    /bin/bash -c "$1" )
    echo "Launched docker image: asorgen/gg-tidyr:v5"
    echo "To execute module: GenMod"
    echo "Docker container id: $containerId"
    echo "${SCRIPT_ID}:docker:${containerId}" >> ${scriptDir}/MAIN_00_TaxaMetaMerge.sh_Started
    docker inspect ${containerId}
}

function scriptFailed() {
    echo "Line #${2} failure status code [ ${3} ]:  ${1}" >> "${scriptDir}/MAIN_00_TaxaMetaMerge.sh_Failures"
    exit ${3}
}
...
executeLine "spawnDockerContainer ${scriptDir}/00.0_GenMod.sh" ${LINENO}

touch "${scriptDir}/MAIN_00_TaxaMetaMerge.sh_Success"
